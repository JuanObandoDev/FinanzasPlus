...
window.onload = async () => {
  ...
  try {
    ...
    alert("Hubo un problema al cargar las categorias de deudas.");
  }
};
...
document
  ...
  .addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const id_categoria_deuda = await getCategoriaId(categoria);

      const data = {
        id_usuario: id_usuario,
        monto_pagar: monto,
        fecha_vencimiento: fecha,
        id_categoria_deuda: id_categoria_deuda,
        descripcion: descripcion,
      };

      await saveDebt(data);

      alert("La deuda fue guardada exitosamente");
...
async function getDescripcion(categoria, descripcionInput) {
  ...
  const { data, error } = await SUPABASE.from("categorias_deudas")
    .select("descripcion")
    .eq("nombre", categoria)
    .single();
...
async function getCategoriaId(categoria) {
  ...
  return data.id_categoria_deuda;
}

async function saveDebt(data) {
  const { error } = await SUPABASE.from("deudas").insert([data]);
